...
public sealed class LiveScoringServiceTests
{
    ...
    public void BuildBlocks_groups_in_match_category_order_and_ranks_with_lazy_equalizers()
    {
        ...
        Assert.Collection(block.Entries,
            ...
            entry => { Assert.Equal((2, true, "Cara"), (entry.Position, entry.NeedsTieBreaker, entry.Line1)); Assert.DoesNotContain("PB:", entry.Line2); Assert.False(entry.AboveTarget); });
    }
    ...
    public void BuildBlocks_renders_and_highlights_personal_best_when_a_lookup_is_supplied()
    {
        ...
        var personalBests = new Dictionary<Guid, PersonalBestSnapshot> { [participant.Id] = new PersonalBestSnapshot(18, 9.0) };

        var blocks = new LiveScoringService(new ScoringService()).BuildBlocks(match, scope, [], personalBests);

        var entry = Assert.Single(Assert.Single(blocks).Entries);
        Assert.Equal("PB: 18", entry.Line2);
        Assert.True(entry.AboveTarget); // (10 + 9) / 2 = 9.5 average, above the 9.0 personal-best average
    }
