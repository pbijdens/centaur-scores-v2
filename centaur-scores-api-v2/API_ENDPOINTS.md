...
| `GET` | `/api/matches/{id}/export.csv?language=en\|nl` | user | Download match results as CSV. The first column is the name of the device the participant is assigned to (empty when unassigned); rows are ordered by device, then by the participant's order on that device, then by full name (unassigned participants sort last). After device, federation number, full name, and total, columns contain counts for each keyboard key in keyboard order, `Null` for unentered arrows, grouped-end totals as `Split1`, `Split2`, etc., category values in match category order, last name, and a `Signed`/`Ondertekend` column (`Yes`/`Ja` or `No`/`Nee`) translated per `language`. |
