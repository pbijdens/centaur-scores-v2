...
- **Scorecard signing** (see `../documentation/SIGNING-SCORECARDS.md`) mirrors this same queue/retry engine: `recordSignature`/`flushPendingSignatures` and the persisted `pendingSignatures` store are structured like their score-edit counterparts, but with no conflict resolution (a `SCORECARD_SIGNED` response is treated as success, not a failure to keep retrying). A `SCORECARD_SIGNED` value on a score-update conflict entry's `error` field (alongside `SCORE_CONFLICT`/`PARTICIPANT_CONFLICT`) means a queued score edit targeted a participant whose card got signed while the edit was pending — handled as an unconditional drop, never surfaced in `ConflictDialog` (see MEMORY.md for why the generic `SCORE_CONFLICT` handling would have silently mis-applied it).
...
- `ScoreCardView` also renders `components/SignArea.svelte` below the ends list when `match.signatureMode !== 'none'` (see `../documentation/SIGNING-SCORECARDS.md`) — a persistent Sign button, then either the recorded signature images or a read-only notice once `participant.signed`. Once signed, a card is permanently locked (`onArrowTap`/`onKey`/`onDelete` all early-return, the keyboard never reopens for that participant) - there is no anonymous unsign anywhere in this app. **Any full-screen dialog rendered from within `ScoreCardView`'s subtree (`SignConfirmDialog`, `SignatureCaptureDialog`) must stop pointer-event propagation on its own `.overlay` root** — `position: fixed` only detaches a dialog visually, not from the real DOM tree, so an unguarded dialog's pointer events still bubble up into the swipe-to-switch-participant handler above and can hijack a drag gesture inside the dialog as a participant swipe (see MEMORY.md for the signature-capture bug this caused).
