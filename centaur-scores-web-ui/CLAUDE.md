...
**Data flow is parent-owned, not per-view fetching.** `App.svelte`'s `bootstrapSession()` runs on every login and every page load with an existing token: it fetches the profile first (`data.ts`'s `loadProfile()`, `GET /api/auth/me`) to resolve which tenant is active (see the tenant-selection entry in MEMORY.md), re-mints the token for that tenant, then calls `loadData()` to eager-load matches, competitions, current tenant, categories, participant lists, and templates in one pass (categories/participant lists come back with nested values/members already included) — profile is deliberately not part of that batch, since resolving it comes first and every other call needs a tenant already selected. Views receive data and an `api: ApiClient` instance as props, call mutation methods on `api`, then invoke an `onChanged`/`onSaved` callback prop so the parent refetches (`refreshCategories()`, `refreshParticipantLists()`, `refreshTemplates()`, etc.) — views do not mutate parent-owned arrays directly except for a few documented optimistic-update cases (see `MatchParticipantView` in MEMORY.md).
...
- **Every `.sticky-actions` save/apply button must report its result via a blocking `alert(message)`** in the `save()`/`apply()` function's try/catch (success and error alike) — not a toast, and not inline text the user could miss above the sticky bar. This is a deliberate UX choice: the user must actively acknowledge a save result before continuing, matching this codebase's existing modal-acknowledgment pattern (`confirm()` for destructive actions, `alert()` in `SessionExpiryWatcher`) rather than a custom modal component or an auto-dismissing corner notification. Don't add a per-view `saveMessage`/`saveError` paragraph next to a sticky-actions button. An inline `.error`/`.success` paragraph is still fine for feedback tied to a *different* action on the same page (e.g. file validation, a secondary form) — only the sticky save/apply button itself must use `alert()`.
