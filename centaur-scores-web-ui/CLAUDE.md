...
- Universal list-rendering rule (applies wherever a participant-list member appears in a selection UI): render as "full name (concatenated category values)" using the tenant's full `categories` list, and always exclude members with `isActive === false`.
