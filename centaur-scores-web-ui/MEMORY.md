...
- `MatchTemplate.configurationJson` is a free-form JSON blob the backend never parses/validates; its shape (categoryOrder, keyboard, disabledKeyRules, scoringRules, liveScopes) is defined and owned entirely by the frontend in `src/lib/templateConfig.ts` (parse/default helpers) and `src/lib/types.ts` (`TemplateConfiguration` and friends). Keyboard keys are `{ keyId, label, color, value }` (per root `documentation/DESIGN.md`'s "Matches" section); the special delete/wipe key has `keyId`/`color`/`value` all `null`. Scoring rules are an ordered list of `{ type: 'total' | 'countKey', keyId? }` tie-breakers. Live scopes mirror the real `LiveScoreScope` entity's fields.
