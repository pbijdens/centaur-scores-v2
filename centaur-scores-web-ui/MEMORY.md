...
- Category management (UC5) and participant list management (UC7) each have dedicated list/detail views (`CategoriesView`/`CategoryDetailView`, `ParticipantListsView`/`ParticipantListDetailView`/`ParticipantMemberView`); `categories` and `participants` were removed from the generic `ManagementView` stub in `types.ts`'s `managementSections` (only `templates` still uses it, pending UC6).
- Categories and participant lists (with members) are eager-loaded in `App.svelte`'s `loadData()` alongside matches/competitions, since the backend already returns nested values/members in one call; after any mutation, call `refreshCategories()`/`refreshParticipantLists()` to refetch rather than mutating local state.
